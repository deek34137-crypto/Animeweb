...
export const animepaheProvider: StreamingProviderInterface = {
  ...
  getStreamInfo: async (animeId: string, episode: number, animeTitle?: string): Promise<EpisodeStreamInfo> => {
    ...
    // For AnimePahe, prefer dub sources as primary if available
    const primarySources = dubSources.length > 0 ? dubSources : subSources;
    ...
    return {
      ...
      dub: dubSources,
      subtitles,
      audioLanguage: hasDub ? 'english' : 'japanese',
      isFallback: false,
